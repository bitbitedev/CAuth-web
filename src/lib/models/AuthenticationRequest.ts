...
export class AuthenticationRequest {
    ...
    constructor(id: string, date: Timestamp, inquirer: any, user: any, status: string, message: string){
        this.id = id;
        this.date = date;
        this.status = status;
    ...
    async getReference():Promise<DocumentReference<AuthenticationRequest>>{
        return doc(collection(db, "authentication-request"), this.id).withConverter(authenticationRequestConverter);
    }
...
export const authenticationRequestConverter = {
    ...
    fromFirestore: (snapshot: any, options: any) => {
        ...
        return new AuthenticationRequest(snapshot.id, data.date, data.inquirerRef, data.userRef, data.status, data.message);
    }
}