...
<div class="card card-compact md:card-normal max-w-full bg-base-100 shadow-2xl">
	<div class="card-body">
		...
		<p>These are your most recent logins to an other platform.</p>
		...
		<div class="flex flex-wrap gap-2 mt-2">
			{#each data.sessions as session}
				<div
					class="card card-compact bg-base-200 border-[1px]"
					class:border-error={!session.valid}
					class:border-success={session.valid}>
					<div class="card-body">
						<div class="card-title">
							{session.platform.name}
							<span
								class="badge badge-outline"
								class:badge-error={!session.valid}
								class:badge-success={session.valid}>
								{#if session.valid}
									active
						...
						<span>
							Logged in: {new Date(session.createdAt).toLocaleDateString(data.lang, dateSettings)}
						</span>
						<span>
						...
						{#if session.valid}
							<div class="card-actions justify-end mt-2">
								<form action="?/end" method="POST">
									<button class="btn btn-primary" name="session" value={session.id.split(':')[1]}>
										End Session
									</button>
								</form>
							</div>
