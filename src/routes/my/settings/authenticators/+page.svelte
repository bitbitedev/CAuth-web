			...
			<div class="card-body">
				<div class="card-title p-2">
					...
					<button
						...
						onclick="modal_edit_{authenticator.id.split(':')[1]}.showModal();">
						<Edit />
					</button>
					<button
						class="btn btn-error btn-circle btn-xs text-white"
						onclick="modal_delete_{authenticator.id.split(':')[1]}.showModal();">
						<Delete />
					</button>
				</div>
					...
					<div class="flex flex-col gap-4">
						<div class="flex flex-row gap-4">
							...
							<div class="flex flex-col gap-1">
								...
								<div class="text-sm">
									{new Date(authenticator.createdAt).toLocaleDateString(data.lang, {
										month: 'long',
										day: 'numeric',
										hour: '2-digit',
										minute: '2-digit'
					...
					<form action="?/edit" method="POST" use:enhance>
						...
						<div class="modal-action">
							<button
								...
								onclick="modal_edit_{authenticator.id.split(':')[1]}.close();">
								Close
							</button>
							<button
								class="btn btn-primary"
								name="id"
								value={authenticator.id.split(':')[1]}
								onclick="modal_edit_{authenticator.id.split(':')[1]}.close();">
								Confirm
							</button>
						</div>
				...
				<Modal id="modal_delete_{authenticator.id.split(':')[1]}" title="Delete authenticator">
					<p class="mb-1 text-error">
						If you delete this authenticator you can no longer login on the device that registered
						this authenticator. You can always add a new authenticator <a href="/new-device">
							here
						</a>
						.
					</p>
					<form action="?/deleteAuthenticator" method="POST" use:enhance>
						<div class="modal-action">
							<button
								...
								on:click|preventDefault
								onclick="modal_delete_{authenticator.id.split(':')[1]}.close();">
								Close
							</button>
							<button
								class="btn btn-error"
